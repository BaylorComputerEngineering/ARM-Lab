...
You are to:
\begin{enumerate}
\item Finish the testbench in Listing~\ref{code:register_test} by testing several additional cases.  Please add the following test cases:
\begin{enumerate}
	\item Change the value of D at different points during the clock cycle and check the results to ensure that they match the expected behavior of a register.  Please add 3 additional changes in D at different time intervals to fulfill this requirement.
	\item Add a case where D is set for multiple clock cycles and ensure that the results match the expected behavior.
	\item Test the reset functionality to ensure that it clears the output Q when reset is asserted.
	\item Verify that the register returns to normal operation once the reset is de-asserted.
\end{enumerate} 
